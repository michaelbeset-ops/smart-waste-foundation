...
function b_advice_setup() {
    ...
    register_nav_menus( [
        'primary' => __( 'Primair menu', 'b-advice' ),
    ] );

    add_post_type_support( 'post', 'excerpt' );
}
add_action( 'after_setup_theme', 'b_advice_setup' );

/* ==============================
   ENQUEUE STYLES
   ============================== */
function b_advice_scripts() {
    wp_enqueue_style( 'b-advice-style', get_stylesheet_uri(), [], '1.0.0' );
}
add_action( 'wp_enqueue_scripts', 'b_advice_scripts' );

/* ==============================
   CONTACT FORM HANDLER
   ============================== */
function b_advice_handle_contact_form() {
    if ( ! isset( $_POST['ba_contact_nonce'] ) ) return null;
    if ( ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['ba_contact_nonce'] ) ), 'ba_contact_form' ) ) {
        return 'error';
    }
    ...
    if ( ! $naam || ! $email || ! $bericht || ! is_email( $email ) ) {
        return 'error';
    }

    $to      = get_option( 'admin_email' );
    $subject = 'Contactformulier B-Advice van ' . $naam;
    $body    = "Naam: {$naam}\nE-mail: {$email}\nTelefoon: {$telefoon}\n\nBericht:\n{$bericht}";
    $headers = [
    ...
    return wp_mail( $to, $subject, $body, $headers ) ? 'success' : 'error';
}

/* ==============================
   WIDGET AREAS
   ============================== */
function b_advice_widgets_init() {
    register_sidebar( [
        'name'          => __( 'Zijbalk Nieuws', 'b-advice' ),
        'id'            => 'sidebar-nieuws',
        'before_widget' => '<div class="ba-widget">',
        'after_widget'  => '</div>',
        'before_title'  => '<h3 class="ba-widget-title">',
        'after_title'   => '</h3>',
    ] );
}
add_action( 'widgets_init', 'b_advice_widgets_init' );

/* ==============================
   EXCERPT LENGTH
   ============================== */
function b_advice_excerpt_length( $length ) {
    return 25;
}
add_filter( 'excerpt_length', 'b_advice_excerpt_length' );

function b_advice_excerpt_more( $more ) {
    return '…';
}
add_filter( 'excerpt_more', 'b_advice_excerpt_more' );

/* ==============================
   HELPER: SVG ICONS
   ============================== */
function ba_icon( $name, $size = 20 ) {
    $icons = [
        ...
        'phone'          => '<svg xmlns="http://www.w3.org/2000/svg" width="' . $size . '" height="' . $size . '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 13a19.79 19.79 0 0 1-3.07-8.67A2 2 0 0 1 3.54 2h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 9.91a16 16 0 0 0 6.18 6.18l1.27-1.27a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>',
        'map-pin'        => '<svg xmlns="http://www.w3.org/2000/svg" width="' . $size . '" height="' . $size . '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 10c0 4.993-5.539 10.193-7.399 11.799a1 1 0 0 1-1.202 0C9.539 20.193 4 14.993 4 10a8 8 0 0 1 16 0"/><circle cx="12" cy="10" r="3"/></svg>',
        'monitor'        => '<svg xmlns="http://www.w3.org/2000/svg" width="' . $size . '" height="' . $size . '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect width="20" height="14" x="2" y="3" rx="2"/><path d="M8 21h8m-4-4v4"/></svg>',
        'briefcase'      => '<svg xmlns="http://www.w3.org/2000/svg" width="' . $size . '" height="' . $size . '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 20V4a2 2 0 0 0-2-2h-4a2 2 0 0 0-2 2v16"/><rect width="20" height="14" x="2" y="6" rx="2"/></svg>',
        ...
        'bar-chart3'     => '<svg xmlns="http://www.w3.org/2000/svg" width="' . $size . '" height="' . $size . '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M3 3v18h18"/><path d="M18 17V9"/><path d="M13 17V5"/><path d="M8 17v-3"/></svg>',
        ...
        'truck'           => '<svg xmlns="http://www.w3.org/2000/svg" width="' . $size . '" height="' . $size . '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 17H3a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11a2 2 0 0 1 2 2v3"/><rect width="7" height="7" x="14" y="12" rx="1"/><circle cx="17.5" cy="21.5" r="2.5"/><circle cx="6.5" cy="21.5" r="2.5"/></svg>',
        ...
        'user'            => '<svg xmlns="http://www.w3.org/2000/svg" width="' . $size . '" height="' . $size . '" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M19 21v-2a4 4 0 0 0-4-4H9a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>',
...
/* ==============================
   WOOCOMMERCE SUPPORT
   ============================== */
function b_advice_woocommerce_support() {
...
add_action( 'after_setup_theme', 'b_advice_woocommerce_support' );

/* ==============================
   WOOCOMMERCE: CART ICON
   ============================== */
function ba_cart_link() {
    if ( ! function_exists( 'WC' ) || ! WC()->cart ) return '';
    $count = WC()->cart->get_cart_contents_count();
    $badge = $count > 0
        ? '<span class="ba-cart-count" aria-label="' . $count . ' artikel(en)">' . $count . '</span>'
        : '';
    return sprintf(
        '<a href="%s" class="ba-cart-link" aria-label="Winkelwagen (%d artikelen)">%s%s</a>',
        esc_url( wc_get_cart_url() ),
        $count,
        ba_icon( 'shopping-cart', 20 ),
        $badge
    );
}

// Keep cart count up-to-date via AJAX fragments
add_filter( 'woocommerce_add_to_cart_fragments', function ( $fragments ) {
    ob_start();
    echo ba_cart_link(); // phpcs:ignore WordPress.Security.EscapeOutput
    $fragments['.ba-cart-link'] = ob_get_clean();
    return $fragments;
} );
...
add_filter( 'woocommerce_breadcrumb_defaults', function ( $defaults ) {
    $defaults['wrap_before'] = '<nav class="ba-wc-breadcrumb" aria-label="Kruimelpad"><ul>';
